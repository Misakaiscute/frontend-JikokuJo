...
{#if trip !== null}
    <div class="flex-[0_0_40] w-full my-0.25 bg-white rounded-md flex items-center">
        <!-- svelte-ignore a11y_click_events_have_key_events -->
        <div
            role="button" tabindex="0"
            aria-label="{trip.headsign}"
            style="--route-color: {'#' + (routeAssociated?.color ?? '000000')}"
            class="flex-[1_0_auto] w-full px-2 grid grid-cols-5 grid-rows-2 gap-1 rounded-md border-2 border-[var(--route-color)]
        ...
        >
            <div class="w-full h-full col-[1/4] row-[1/2] flex justify-center items-center">
                <p class="flex-[1_0_auto] text-lg h-full truncate">{trip.headsign}</p>
            </div>
            <div class="w-full h-full col-[1/2] row-[2/2] flex justify-center items-center">
                <p class="w-full h-full text-sm truncate">{routeAssociated?.route_short_name}</p>
            </div>
...
{:else}
    <div class="h-14 w-full my-0.25 bg-white border-2 border-zinc-200 rounded-md flex items-center">
